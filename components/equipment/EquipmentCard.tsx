  ...
  return (
    <Link href={`/equipment/${equipment.id}`} className="block">
      <div className="relative">
        <ImagePlaceholder rounded="rounded-lg" />
        {rented && (
      ...
      <div className="mt-2.5 text-[12px] font-medium text-text-secondary">{equipment.category}</div>
      ...
      <div className="mt-1.5 text-[14px] font-extrabold text-ink">
        {formatDailyPrice(equipment.pricePerDay)}
      </div>
    </Link>
  );
}
